function [Q,W,K] = lmi_estabilizacao2 (N,R,mu,fi,V)
...
for t1=1:nv
  for ik=1:ri,
    for ii=1:ri,
        ...
        a11=-A{1,ik,ii}*W-W'*A{1,ik,ii}'+B2{1,ik}*Y{1,ik}+Y{1,ik}'*B2{1,ik}';
        a21=Q{1,ik}+W'-mu*(A{1,ik,ii}*W-B2{1,ik}*Y{1,ik});
        a22=mu*(W+W');
        LMI{1,t1,ik,ii}=[a11 a21'
                         a21 a22];
        % Variaveis matriciais para a incerteza 2
        a11=-A{2,ik,ii}*W-W'*A{2,ik,ii}'+B2{1,ik}*Y{1,ik}+Y{1,ik}'*B2{1,ik}';
        a21=Q{1,ik}+W'-mu*((A{2,ik,ii}*W-B2{1,ik}*Y{1,ik}));
        a22=mu*(W+W');
